...
----------------------------------------------------------------------------
 Файловый      `FileCache`      Файловая система через `lphenom/storage ^0.1` 
 База данных   `DbCache`        MySQL через `lphenom/db ^0.1`                 
 В памяти      `InMemoryCache`  PHP-массив (время жизни процесса)             
 Redis         `RedisCache`     Redis через `lphenom/redis ^0.1`              
